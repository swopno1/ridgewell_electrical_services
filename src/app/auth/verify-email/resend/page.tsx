...
function ResendVerificationContent() {
  ...
  const handleFormSubmit = async (data: ResendFormValues) => {
    setError(null);
    setIsLoading(true);
    setSubmittedEmail(data.email);

    try {
      const response = await resendVerificationAction(data);
        ...
        <Card className="max-w-md w-full border border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-950 p-6 text-center">
          <CardHeader className="flex flex-col items-center">
            ...
            <CardDescription className="text-slate-500">
              If an unverified account is registered to {submittedEmail}, we have sent a verification link.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
            <p>
              Please click the link in that email to verify your email address. The link is valid for 24 hours.
            </p>
            <div className="p-3 text-xs bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30 rounded-lg text-amber-700 dark:text-amber-400 flex items-start gap-2">
              ...
              <span className="text-left leading-normal">
                <strong>Note:</strong> In development mode, check the <strong>terminal console log</strong> of your dev server to copy and paste the verification link!
              </span>
            </div>
          </CardContent>
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <Card className="border border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-950">
          <CardHeader className="space-y-1">
            <CardTitle className="text-xl font-bold">Resend Verification</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
              ...
              >
                {isLoading ? 'Sending link...' : 'Resend Verification'}
              </Button>
            </form>
          </CardContent>
